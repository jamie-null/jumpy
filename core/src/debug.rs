...
pub fn install(session: &mut GameSession) {
    session
        ...
        .add_system_to_stage(CoreStage::Last, debug_render_block_regions);
}
...
fn debug_render_block_regions(
    settings: Res<DebugSettings>,
    entities: Res<Entities>,
    regions: Comp<BlockRegion>,
    transforms: Comp<Transform>,
    mut paths: CompMut<Path2d>,
) {
    let path_for_region = |rotation: f32, region: &BlockRegion| {
        let rect = Rect::new(0.0, 0.0, region.size.x, region.size.y);
